...
public class WebViewObject : MonoBehaviour
{
	...
	[DllImport("WebView")]
	private static extern void _WebViewPlugin_LoadPostURL(
		IntPtr instance, string url, string postData);
	[DllImport("WebView")]
	...
	public void LoadPostURL(string url, string postData)
	{
		#if UNITY_EDITOR || UNITY_STANDALONE_OSX || UNITY_IPHONE
		if (webView == IntPtr.Zero)
			return;
		_WebViewPlugin_LoadPostURL(webView, url, postData);
		#elif UNITY_ANDROID
		if (webView == null)
			return;
		Debug.Log("Unity LoadPostURL: "+url+" postData: "+postData);
		webView.Call("LoadPostURL", url, postData);
		#elif UNITY_WEBPLAYER
		Application.ExternalCall("unityWebView.loadPostURL", name, url, postData);
		#endif
	}
	...
	public void EvaluateJS(string js)
	{
		...
		if (webView == null)
			return;
		webView.Call("EvaluateJS", "javascript:" + js);
		#elif UNITY_WEBPLAYER
	...
	public void SetKeyboardVisible(string shouldVisible){
...
//		Debug.Log("(int)webView: "+(int)webView);
