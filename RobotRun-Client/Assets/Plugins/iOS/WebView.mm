...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    UnitySendMessage([gameObjectName UTF8String],
                     "LoadComplete", [[webView.request.URL.absoluteString substringFromIndex:6] UTF8String]);
}
...
- (void)loadPostURL:(const char *)url postData:(const char *)postData
{
    NSString *urlStr = [NSString stringWithUTF8String:url];
    NSString *postDataStr = [NSString stringWithUTF8String:postData];
    NSURL *nsurl = [NSURL URLWithString:urlStr];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:nsurl];
    [request setHTTPMethod: @"POST"];
    [request setHTTPBody: [postDataStr dataUsingEncoding: NSUTF8StringEncoding]];
    [webView loadRequest:request];
}
...
extern "C" {
    ...
    void _WebViewPlugin_LoadPostURL(void *instance, const char *url, const char *postData);
...
{
    WebViewPlugin *webViewPlugin = (WebViewPlugin *)instance;
    [webViewPlugin loadPostURL:url postData:postData];
}
